  document.addEventListener("contextmenu", (e) => {
    ...
    const API_BIRTHDAY_NAME_URL = "https://apianiversario.capef.com.br/Consultar/PorNome?Nome={NAME_VALUE}";
    const API_BIRTHDAY_DATE_URL = "https://apianiversario.capef.com.br/Consultar/Dia/{DAY_VALUE}/Mes/{MONTH_VALUE}";
    ...
    function atualizarDias() {
        ...
        // Converter o valor de mes para um número inteiro
    ...
    const makeAuthorizedRequest = async(url,apiName)=>{
        ...
        return await fetch(url, options).then((response)=>response.json()).catch((error)=>{
            preloader.style.display = "none";
        }
        );
    }
    ...
    const birthdayResponse = async({name, date})=>{
        ...
        if (name) {
            ...
            url = API_BIRTHDAY_DATE_URL.replace("{DAY_VALUE}", date.day).replace("{MONTH_VALUE}", date.month);
        }
    ...
    function createListItem(element) {
        ...
        dobIcon.src = "https://assets.website-files.com/639787ef2df6f2cf951c2cba/6423a6d521de6a4d059dd8f8_Icon%20Right.svg";
        // Format dob
        ...
        // Get dob month name
        const dobMonthName = document.querySelector("select#mes option[value='" + dobMonth + "']").textContent;

        const dobText = document.createTextNode(dobDay + " " + dobMonthName.toUpperCase());
    ...
    async function searchBirthdays() {
        ...
        const response = await birthdayResponse({
            name,
            date
        });
